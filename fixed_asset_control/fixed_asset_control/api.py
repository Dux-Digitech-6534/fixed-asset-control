...
def get_dashboard_data(filters=None):
	...
	return {
		"total_asset_qty": sum(flt(row.get("qty")) for row in bin_rows),
...
def get_asset_details(asset, company=None):
	...
	total = frappe.get_all("Asset Bin", filters=filters, fields=["sum(qty) as qty"])[0].qty or 0
	bins = [
		row
...
def submit_move_asset(data):
	...
	details = get_asset_details(data.get("asset_name"), data.get("company"))
